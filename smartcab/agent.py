...
class LearningAgent(environment.Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.5):
        ...
        self.trial_number = 1
        self.actions_chosen_learning = 0
        self.actions_chosen_non_learning = 0
    ...
    def reset(self, destination=None, testing=False):
        ...
        self.epsilon = (
            0.
            if percent_mapped > 95. or (self.trial_number > 1000)
            else 1.)
    ...
    def build_state(self):
        ...
        return (
            ...
            inputs['right'],  # the intended direction of travel for a vehicle to the Smartcab's right.
    ...
    def get_maxQ(self, state):
        ...
        # More than one state could have the max Q.  If we are still learning,
        # then pick randomly.

        state_q = self.Q[state]
        max_q_val = max(state_q[action] for action in state_q)
        max_q_actions = [
            action for action in state_q if state_q[action] == max_q_val]
    ...
    def choose_action(self, state):
        ...
        if not self.learning or random.random() < self.epsilon:
            ...
            # Try to preferentially explore states that we have never seen before.
            unknown_actions = [action for action in q_state if q_state[action] == 0]
            if not unknown_actions:
                # We've seen everything ...
                #   Reinforce the best one?
                #   Pick at random?
                #
                #   Random picking could cause sub-optimal states to rise above
                #   optimal ones (if they happen to get picked _way_ more often)
                #   so, let's just keep reinforcing the best state...
                self.actions_chosen_non_learning += 1
                return self.get_maxQ(state)

            self.actions_chosen_learning += 1
...
def run():
    ...
    print 'Number of actions chosen while exploring:', agent.actions_chosen_learning
    print 'Number of "optimal" actions chosen:', agent.actions_chosen_non_learning
